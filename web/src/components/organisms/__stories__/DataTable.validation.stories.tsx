...
import type { ColDef, CellValueChangedEvent, GridApi, RowClassRules } from 'ag-grid-community';
import { z } from 'zod';
...
const RowSchema = z.object({
  ...
  amount: z.number().min(0, 'Amount must be >= 0').max(1_000_000, 'Amount too large'),
  percent: z.number().min(0, 'Percent must be >= 0%').max(1, 'Percent must be <= 100%'),
  active: z.boolean(),
});
type RowErrs = Partial<Record<keyof Row, string>>;

const colsFactory = (errorsRef: React.MutableRefObject<Map<number, RowErrs>>): ColDef<Row>[] => [
  { headerName: 'ID', field: 'id', dataType: 'number', maxWidth: 100, sortable: true },
  {
    headerName: 'Name',
    field: 'name',
    editable: true,
    ...
    cellClassRules: { 'cell-error': (p) => !!errorsRef.current.get(p.data.id)?.name },
  },
  {
    headerName: 'Amount',
    field: 'amount',
    ...
    valueFormatter: (p) => new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 2 }).format(p.value ?? 0),
    valueParser: (p) => {
    ...
    cellClassRules: { 'cell-error': (p) => !!errorsRef.current.get(p.data.id)?.amount },
  },
  {
    headerName: 'Percent',
    field: 'percent',
    ...
    filter: 'agNumberColumnFilter',
    valueFormatter: (p) =>
      new Intl.NumberFormat(undefined, { style: 'percent', minimumFractionDigits: 0, maximumFractionDigits: 2 })
        .format(p.value ?? 0),
    valueParser: (p) => {
    ...
    cellClassRules: { 'cell-error': (p) => !!errorsRef.current.get(p.data.id)?.percent },
  },
  {
    headerName: 'Active',
    field: 'active',
    ...
    cellClassRules: { 'cell-error': (p) => !!errorsRef.current.get(p.data.id)?.active },
  },
];
...
export const InlineValidationSaveRevert: Story = {
  render: () => {
    ...
    const originalRef = React.useRef<Map<number, Row>>(new Map(initialRows.map(r => [r.id, { ...r }])));
    ...
    const updateErrors = (rowId: number, update: (prev: RowErrs) => RowErrs) => {
      const prev = errorsRef.current.get(rowId) || {};
    ...
    const validateField = (data: Row, field: keyof Row): string | undefined => {
      ...
      const issues = parsed.error.issues as Array<{ path: (keyof Row)[], message: string }>;
    ...
    const onCellValueChanged = (e: CellValueChangedEvent<Row>) => {
      ...
      if (msg) {
        ...
        const reverted = rows.map(r => r.id === id ? { ...r, [field]: oldValue } : r);
        setRows(reverted);
    ...
    const revertSelected = () => {
      ...
      const reverted = rows.map(r => selectedIds.has(r.id) ? (originalRef.current.get(r.id) || r) : r);
      setRows(reverted);
    ...
    const onGridReady = (p: { api: GridApi<Row> }) => { apiRef.current = p.api; };
    ...
    return (
      <div style={{ display: 'grid', gap: 8 }}>
        <div style={{
          ...
          padding: 8, border: '1px solid #ddd', borderRadius: 8, background: 'rgba(0,0,0,0.02)'
        }}>
          <strong>{dirtyIdsRef.current.size} dirty</strong>
          <button onClick={saveAll}>Save all (⌘/Ctrl+S)</button>
          <button onClick={revertSelected} disabled={selected.length === 0}>Revert selected</button>
        </div>
