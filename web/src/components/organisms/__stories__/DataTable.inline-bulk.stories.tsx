...
export const InlineEditingAndBulkActions: Story = {
  render: () => {
    ...
    const applyBulk = (fn: (r: Row) => Row) => {
      setRows(prev => prev.map(r => selected.includes(r.id) ? fn(r) : r));
    };
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {selected.length > 0 && (
          <div style={{
            ...
            <button onClick={() => applyBulk(r => ({ ...r, active: true }))}>Activate</button>
            <button onClick={() => applyBulk(r => ({ ...r, active: false }))}>Deactivate</button>
            <button onClick={() => setRows(prev => prev.filter(r => !selected.includes(r.id)))}>Delete</button>
          </div>
        )}
