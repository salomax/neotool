...
import type { ColDef, GridApi, ColumnApi, ICellEditorParams } from "ag-grid-community";
...
function PercentEditor(props: ICellEditorParams) {
  ...
  return (
    <TextField
      ...
      onKeyDown={(e) => { if (e.key === "Enter") (props as any).stopEditing?.(); }}
      sx={{ width: 100 }}
    />
  );
}
...
export default function EnterpriseTable() {
  ...
  const columnDefs = React.useMemo<ColDef<Row>[]>(() => [
    { headerName: "", field: "__sel__" as any, checkboxSelection: true, headerCheckboxSelection: true, width: 48, pinned: "left", sortable: false, filter: false },
    { headerName: "ID", field: "id", filter: "agNumberColumnFilter", width: 100 },
    { headerName: "Name", field: "name", filter: "agTextColumnFilter", flex: 1, editable: true },
    { headerName: "Percent", field: "percent", filter: "agNumberColumnFilter", editable: true, cellEditor: PercentEditor as any,
      valueFormatter: (p) => p.value == null || p.value === "" ? "" : `${(Number(p.value) * 100).toFixed(0)}%` },
    { headerName: "Amount", field: "amount", filter: "agNumberColumnFilter", editable: true,
      valueFormatter: (p) => p.value == null ? "" : p.value.toLocaleString(undefined, { style: "currency", currency: "USD" }) }
  ], []);

  const defaultColDef = React.useMemo<ColDef>(() => ({ sortable: true, resizable: true }), []);
  ...
  const handleSavePreset = () => { const s = colApi.current?.getColumnState?.(); if (s) savePreset(PRESET_KEY, s); };
  const handleLoadPreset = () => { const st = loadPreset(PRESET_KEY); if (st) colApi.current?.applyColumnState?.({ state: st, applyOrder: true }); };
  const handleResetPreset = () => { clearPreset(PRESET_KEY); colApi.current?.resetColumnState?.(); };
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h5" sx={{ mb: 2 }} data-testid="enterprise-title">Enterprise Table</Typography>
      <Stack direction="row" spacing={1} sx={{ mb: 1 }} alignItems="center">
        ...
        />
        <Box sx={{ flexGrow: 1 }} />
        <Button variant="outlined" onClick={handleDeleteSelected} data-testid="enterprise-delete">Delete Selected</Button>
        <Button variant="outlined" onClick={handleExportCsv} data-testid="enterprise-export">Export CSV</Button>
        <Button variant="outlined" onClick={handleSavePreset}>Save Preset</Button>
        <Button variant="outlined" onClick={handleLoadPreset}>Load Preset</Button>
        <Button variant="text" onClick={handleResetPreset}>Reset</Button>
      </Stack>
