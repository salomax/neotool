...
const ExternalCreateDemo: React.FC = () => {
  const [rows, setRows] = React.useState<Row[]>(Array.from({ length: 6 }, (_, i) => ({ id: i + 1, name: `User ${i + 1}`, email: `user${i + 1}@example.com` })));
  const [open, setOpen] = React.useState(false);

  const Form: React.FC<{ onSuccess: (r: Row) => void }> = ({ onSuccess }) => {
    const methods = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { name: '', email: '' } });
    ...
    <>
      <CrudPage<Row>
        ...
        renderForm={({ mode, item, close }) => {
          const methods = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { name: item?.name ?? '', email: item?.email ?? '' } });
          const onSubmit = (data: FormData) => { /* demo only */ close(); };
          return (
            <FormProvider {...methods}>
              <form onSubmit={methods.handleSubmit(onSubmit)}>
                <Stack spacing={2}>
