...
type Row = { id: number; name: string; email: string; role: Role; status: Status; createdAt: string; amount: number };
...
const names = ['Alice','Bob','Carol','Dave','Eva','Frank','Grace','Hugo','Ivy','John','Katy','Leo','Mila','Noah','Olga','Paul','Quinn','Rita','Sam','Tina','Uma','Vic','Wes','Xena','Yuri','Zoe'];
...
function makeData(count = 1000): Row[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const email = `${first}.${lastInitial.replace('.','')}${i}@example.com`.toLowerCase();
...
function applyFilters(rows: Row[], params: URLSearchParams): Row[] {
  ...
  if (search) {
    out = out.filter(r => r.name.toLowerCase().includes(search) || r.email.toLowerCase().includes(search));
  }
...
function applySort(rows: Row[], sort: string | null): Row[] {
  if (!sort) return rows;
  const parts = sort.split(',').map(p => p.trim()).filter(Boolean);
